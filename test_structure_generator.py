...
def setup_test_environment(base_dir):
    ...
    create_file(
        os.path.join(source2_dir, "original_doc.txt"), dup1_content, mtime=older_time
    )
    print(f"  - Created: source_Y2/original_doc.txt (oldest - original)")

    create_file(
        os.path.join(source1_dir, "copy_of_doc.txt"), dup1_content, mtime=old_time
    )
    ...
    create_file(
        os.path.join(source1_dir, "file with spaces.txt"), "Testing spaces in filename"
    )
    print(f"  - Created: source_Y1/file with spaces.txt")

    # File with multiple extensions
    create_file(os.path.join(source2_dir, "archive.tar.gz.bak"), "Fake archive backup")
    print(f"  - Created: source_Y2/archive.tar.gz.bak (temp extension)")

    # Very long filename
    long_name = "a" * 100 + ".txt"
    create_file(os.path.join(source1_dir, long_name), "File with very long name")
    print(f"  - Created: source_Y1/{long_name[:30]}... (long name)")

    # File with unicode characters
    create_file(
        os.path.join(target_dir, "załącznik.txt"), "Polish characters in filename"
    )
