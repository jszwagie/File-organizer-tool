...
def setup_test_environment(base_dir):
    ...
    print("  - Created: target_X/empty_file.txt (empty)")

    create_file(os.path.join(source1_dir, "another_empty.dat"), content="")
    print("  - Created: source_Y1/another_empty.dat (empty)")
    ...
    # Duplicate set 1: Same content, original should be oldest
    dup1_content = (
        "This is the original content that gets duplicated.\n" * 10)

    create_file(
        os.path.join(source2_dir, "original_doc.txt"),
        dup1_content, mtime=older_time)
    print(
        "  - Created: source_Y2/original_doc.txt "
        "(oldest - original)")

    create_file(
        os.path.join(source1_dir, "copy_of_doc.txt"),
        dup1_content, mtime=old_time)
    print("  - Created: source_Y1/copy_of_doc.txt (duplicate)")

    create_file(
        os.path.join(target_dir, "another_copy.txt"),
        dup1_content, mtime=now)
    print(
        "  - Created: target_X/another_copy.txt "
        "(newest duplicate)")
    ...
    create_file(
        os.path.join(target_dir, "data.bin"),
        dup2_content, mtime=older_time)
    print("  - Created: target_X/data.bin (oldest - in target)")

    create_file(
        os.path.join(source1_dir, "data_backup.bin"),
        dup2_content, mtime=now)
    print("  - Created: source_Y1/data_backup.bin (duplicate)")
    ...
    create_file(
        os.path.join(subdir1, "app.conf"),
        dup3_content, mtime=older_time)
    print("  - Created: source_Y1/config/app.conf (oldest)")

    create_file(
        os.path.join(subdir2, "app.conf"),
        dup3_content, mtime=now)
    print(
        "  - Created: source_Y2/settings/app.conf "
        "(duplicate, same name)")
    ...
    print("  - Created: target_X/report.txt (v1 - oldest)")
    ...
    print("  - Created: source_Y1/report.txt (v2 - middle)")
    ...
    print("  - Created: source_Y2/report.txt (v3 - newest)")
    ...
    create_file(
        ...
        mtime=older_time)
    print("  - Created: target_X/docs/notes.md (old version)")
    ...
    create_file(
        ...
        mtime=now)
    print(
        "  - Created: source_Y1/archive/notes.md "
        "(new version)")
    ...
    normal_files = [
        ...
        (source1_dir, "unique_file.txt",
         "This content is unique to source1"),
        (source2_dir, "another_unique.txt",
         "This content is unique to source2"),
    ]
    ...
    print("  - Created: source_Y1/file with spaces.txt")
    ...
    create_file(
        ...
        "Fake archive backup")
    print(
        "  - Created: source_Y2/archive.tar.gz.bak "
        "(temp extension)")
    ...
    print("  - Created: target_X/załącznik.txt (unicode)")

    # Hidden file
    create_file(os.path.join(source1_dir, ".hidden_config"), "hidden=true")
    print("  - Created: source_Y1/.hidden_config (hidden)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "test_env"),
